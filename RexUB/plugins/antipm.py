...
from RexUB.modules.helpers.program import get_arg, denied_users
import RexUB.modules.databases.pmpermit_db as kdy
...
async def antipm(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "off":
        await kdy.set_pm(False)
        await message.edit("**PM Guard Deactivated**")
    if arg == "on":
        await kdy.set_pm(True)
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await kdy.set_permit_message(kdy.PMPERMIT_MESSAGE)
        await message.edit("**Anti_PM message set to default**.")
        return
    await kdy.set_permit_message(f"`{arg}`")
...
async def setblkmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await kdy.set_block_message(kdy.BLOCKED)
        await message.edit("**Block message set to default**.")
        return
    await kdy.set_block_message(f"`{arg}`")
...
async def allow(client, message):
    ...
    pmpermit, pm_message, limit, block_message = await kdy.get_pm_settings()
    await kdy.allow_user(chat_id)
